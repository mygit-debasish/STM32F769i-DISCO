...
int main(void)
{
  ...
  /* starting the ADC1 in with EOC interrupt Enable ✅*/
  //HAL_ADC_Start_IT(&hadc1);

  /* starting DMA2 for ADC transfer
   * DO NOT call HAL_ADC_Start_IT() along with HAL_ADC_Start_DMA() ❌
   * */
  HAL_ADC_Start_DMA(&hadc1, (uint32_t*)sensorData, SENSOR_DATA_LEN);
  ...
  while (1)
  {
		if (EOCFlagON)
		{
			writeFormatData(&huart1, "ADC voltage: %0.2f Volt => Count: %d \r",
					voltADC, dataADC);
			EOCFlagON = 0;
			if(voltADC > 2.0)
				ToggleGreenLED();

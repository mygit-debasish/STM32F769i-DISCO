...
int main(void)
{
...
#if DMA_MODE
  writetoSerial(&huart1, "Configuring ADC1 in DMA mode  \r\n");

  /* initializing ADC1 in nterrupt Mode */
  ADC1_DMA_Initialization((uint16_t*)sensorData, SENSOR_DATA_LEN);
  ...
  while (1)
  {
	  ...
	  /* Get single byte from ADC1 */
	  AdcCount = ADC1->DR;
	  voltADC = ADC_CountToVolt(AdcCount);
	  writeFormatData(&huart1, "DIRECT: ADC count: %d Voltage: %0.2f \r", AdcCount, voltADC);
	  ...
	  HAL_Delay(250);
		...
		if (EOCFlagON)
		{
			...
			writeFormatData(&huart1,
					"INTR: ADC voltage: %0.2f Volt => Count: %d \r", voltADC,
					AdcCount);
			EOCFlagON = 0;
		...
		if (DTCFlagON)
		{
			...
			voltADC = ADC_CountToVolt(AdcCount);
			writeFormatData(&huart1,
					"DMA: ADC voltage: %0.2f Volt => Count: %d \r", voltADC,
					AdcCount);
			DTCFlagON = 0;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : FMC_D2_Pin FMC_D3_Pin FMC_D1_Pin FMC_D0_Pin
  ...
  /*Configure GPIO pins : ULPI_D6_Pin ULPI_D5_Pin ULPI_D3_Pin ULPI_D4_Pin */
  GPIO_InitStruct.Pin = ULPI_D6_Pin|ULPI_D5_Pin|ULPI_D3_Pin|ULPI_D4_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
  GPIO_InitStruct.Alternate = GPIO_AF10_OTG_HS;

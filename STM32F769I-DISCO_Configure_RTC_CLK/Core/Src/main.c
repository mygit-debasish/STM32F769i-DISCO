...
int main(void)
{
  ...
  writetoSerial(&huart1, "Welcome to RTC configuration ! \r\n");

  /* Sequence of STEPS
   * 1. Enable PWR clock
   * 2. Disable Backup-Domain Protection in PWR Register
   * 3. Disable RTC Write-Protection
   * 4. Enter INIT Mode
   * 5. CONFIGURE RTC Register HERE
   * 6. Exit INIT Mode
   * 7. Re-Enable RTC Write-Protection
   * 8.
   */

  /* Configure RTC for Calender */
  uint8_t curHour;
  uint8_t curMin;
  uint8_t curSec;
  uint8_t curYear;
  ...
  /* Initializing Calender and RTC Alarm Interrupt */
  RTC_Calander_Config();
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : ULPI_D7_Pin ULPI_D6_Pin ULPI_D5_Pin ULPI_D4_Pin */
  GPIO_InitStruct.Pin = ULPI_D7_Pin|ULPI_D6_Pin|ULPI_D5_Pin|ULPI_D4_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
